...
SkeletonGraphPlanner::SkeletonGraphPlanner(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
    : nh_(nh)
...
void SkeletonGraphPlanner::setEsdfLayer(voxblox::Layer<voxblox::EsdfVoxel>* esdf_layer)
{
...
void SkeletonGraphPlanner::setSkeletonLayer(voxblox::Layer<voxblox::SkeletonVoxel>* skeleton_layer) { skeleton_planner_.setSkeletonLayer(skeleton_layer); }

void SkeletonGraphPlanner::setSparseGraph(const voxblox::SparseSkeletonGraph* sparse_graph)
{
...
bool SkeletonGraphPlanner::getPathBetweenWaypoints(
    const mav_msgs::EigenTrajectoryPoint& start, const mav_msgs::EigenTrajectoryPoint& goal, mav_msgs::EigenTrajectoryPoint::Vector* solution)
{
    ...
    bool success = sparse_graph_planner_.getPath(start_point, goal_point, &graph_coordinate_path);
    ...
    success &= skeleton_planner_.getPathInEsdf(start_point, graph_coordinate_path.front(), &exact_start_path);
    success &= skeleton_planner_.getPathInEsdf(graph_coordinate_path.back(), goal_point, &exact_goal_path);

    graph_coordinate_path.insert(graph_coordinate_path.begin(), exact_start_path.begin(), exact_start_path.end());
    graph_coordinate_path.insert(graph_coordinate_path.end(), exact_goal_path.begin(), exact_goal_path.end());
    convertCoordinatePathToPath(graph_coordinate_path, &graph_path);
...
void SkeletonGraphPlanner::convertCoordinatePathToPath(
    const voxblox::AlignedVector<voxblox::Point>& coordinate_path, mav_msgs::EigenTrajectoryPointVector* path) const
{
    CHECK_NOTNULL(path);
    path->clear();
...
void SkeletonGraphPlanner::shortenPath(const mav_msgs::EigenTrajectoryPoint::Vector& path_in, mav_msgs::EigenTrajectoryPoint::Vector* path_out) const
{
